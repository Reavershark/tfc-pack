...
gbuffers_terrain, block, entities, textured, textured_lit, water, and hand handle most of the effects, though there are a few differences between them:
	* textured handles lighting, fog, desaturation, and cross processing.
	* textured_lit does everything textured does, and also applies shading.
		The rest of the programs listed after this also do everything textured_lit (and by extension, textured) does.
	* terrain handles all of the foliage effects, as well as wet dirt, randomized lava brightness, and lava waves (in the nether)
	* block handles end portal rendering
	* entities handles circular shadows and rainbow XP, and applies entity coloring (the red tint when an entity takes damage)
	* water attempts to apply underwater fog to things behind water (though it isn't particularly successful due to the fact that terrain ALSO applies its own fog)
		it also controls clear water, and writes extra data to gnormal and gaux1 which tells composite how to apply the rest of the effects.
	* hand also writes data to gnormal and gaux1 so that blocks that have effects will still do so even in held item form.
		it also controls hand sway

gbuffers_skybasic controls sky gradient rendering, and fancy stars/galaxies.
...
gbuffers_skytextured handles the end background. (world1/gbuffers_skytextured.fsh, to be more specific)
Normally I would do this in composite, but since the rendering pipeline of the modded version is different than the QOL version, that isn't possible here.
As such, the background will always be calculated regardless of weather or not you're looking at it.
The alternative is to not have it render at all behind transparent objects.
Luckily, the end doesn't have as many effects as the overworld, and the terrain in the end isn't as complex as the overworld either, so framerate is about the same.

gbuffers_weather controls waving and soft rain.

composite does some preparations for infinite oceans (applying corrected color/normal data)
composite1 handles the rest of the effects (refraction, ice scatter, etc...)

Composite2/3 are used for blur effects. 2 programs are needed because I'm using a technique called 2-pass blurring.
...
In the nether/end, there's no infinite oceans, so composite takes care of the effects that composite1 normally handles in the overworld.
Similarly, composite1 and 2 handle blur.

That's all the effects, now for the used buffers:

gcolor: Stores the main color. not much to it.
gnormal: Stores normal vectors of the closest transparent object to the camera (used for refractions and reflections)
gaux1: Red and green store block light and skylight of the closest transparent object to the camera.
	Used to prevent applying sky reflections when underground.
	The blue channel also stores block data, used for block-specific effects like water refractions, ice scattering, etc...
gaux3: Deferred stores the depth of the closest opaque object to the camera in red. gbuffers_water uses this for fog.

After composite1 finishes (composite in the nether/end), it writes the final color and blur radius to gaux2.
composite2 (1 in the nether/end) then applies horizontal blur, and writes the result to gaux1.
Lastly: composite3 (2 in the nether/end) applies vertical blur, and writes the result back to gcolor. gcolor is then rendered directly to the screen.
...
Absorption Color (§9blue§r):
	...
	Found in:
		In world-1: composite.fsh/vsh, composite1.fsh, deferred.fsh, final.fsh, gbuffers_armor_glint.vsh, gbuffers_beaconbeam.fsh, gbuffers_beaconbeam.vsh, gbuffers_block.fsh, gbuffers_block.vsh, gbuffers_entities.fsh, gbuffers_entities.vsh, gbuffers_hand.fsh, gbuffers_hand.vsh, gbuffers_terrain.fsh, gbuffers_terrain.vsh, gbuffers_textured.fsh, gbuffers_textured.vsh, gbuffers_water.fsh, gbuffers_water.vsh
		In world0: composite.fsh, composite1.fsh, composite1.vsh, composite2.fsh, deferred.fsh, deferred.vsh, final.fsh, gbuffers_armor_glint.vsh, gbuffers_beaconbeam.fsh, gbuffers_beaconbeam.vsh, gbuffers_block.fsh, gbuffers_block.vsh, gbuffers_entities.fsh, gbuffers_entities.vsh, gbuffers_hand.fsh, gbuffers_hand.vsh, gbuffers_skybasic.fsh, gbuffers_skybasic.vsh, gbuffers_skytextured.fsh, gbuffers_terrain.fsh, gbuffers_terrain.vsh, gbuffers_textured.fsh, gbuffers_textured.vsh, gbuffers_textured_lit.fsh, gbuffers_textured_lit.vsh, gbuffers_water.fsh, gbuffers_water.vsh, gbuffers_weather.fsh, gbuffers_weather.vsh
		In world1: composite.fsh/vsh, composite1.fsh, deferred.fsh, final.fsh, gbuffers_armor_glint.vsh, gbuffers_beaconbeam.fsh, gbuffers_beaconbeam.vsh, gbuffers_block.fsh, gbuffers_block.vsh, gbuffers_entities.fsh, gbuffers_entities.vsh, gbuffers_hand.fsh, gbuffers_hand.vsh, gbuffers_terrain.fsh, gbuffers_terrain.vsh, gbuffers_textured.fsh, gbuffers_textured.vsh, gbuffers_water.fsh, gbuffers_water.vsh
		In world7: composite.fsh/vsh, composite1.fsh, deferred.fsh, final.fsh, gbuffers_armor_glint.vsh, gbuffers_beaconbeam.fsh, gbuffers_beaconbeam.vsh, gbuffers_block.fsh, gbuffers_block.vsh, gbuffers_entities.fsh, gbuffers_entities.vsh, gbuffers_hand.fsh, gbuffers_hand.vsh, gbuffers_skybasic.fsh, gbuffers_terrain.fsh, gbuffers_terrain.vsh, gbuffers_textured.fsh, gbuffers_textured.vsh, gbuffers_water.fsh, gbuffers_water.vsh
...
goldenOffset0 (localization missing):
	...
	Recommended values: (no recommended values provided)
	Found in:
		In world-1: composite.fsh, gbuffers_beaconbeam.fsh, gbuffers_block.fsh
		In world0: composite1.fsh/vsh, deferred.fsh, gbuffers_beaconbeam.fsh, gbuffers_block.fsh
		In world1: composite.fsh/vsh, deferred.fsh, gbuffers_beaconbeam.fsh, gbuffers_block.fsh
		In world7: composite.fsh, gbuffers_beaconbeam.fsh, gbuffers_block.fsh
...
MOON_GLOW_COLOR (localization missing):
	Description: Mixed with sky color based on distance from the moon
	Default Value: vec3(0.075, 0.1,   0.2 )
	Recommended values: (no recommended values provided)
	Found in:
		In world0: composite1.fsh, gbuffers_block.fsh, gbuffers_entities.fsh, gbuffers_hand.fsh, gbuffers_skybasic.fsh, gbuffers_terrain.fsh, gbuffers_textured.fsh, gbuffers_textured_lit.fsh, gbuffers_water.fsh
...
SUN_GLOW_COLOR (localization missing):
	Description: Mixed with sky color based on distance from the sun
